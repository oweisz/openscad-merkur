 ...
 * include <merkur/merkur-bosl2.scad>
 * 
 * inslall BOSL2 library
 * instaluj knihovnu BOSL2
 * https://github.com/BelfrySCAD/BOSL2.git
 * 
 * render BOSL2 variant
 * zobrazeni s knihovnami BOSL2
 *
 */

// lib for full render screw, gear, spring
...
module M1045(){
  ...
  color("black"){
    difference(){
     ...
     color("DarkSlateGray")up(2.5/2)rot(3.5)spur_gear(circ_pitch=1.89, teeth=50, thickness=2.5, shaft_diam=25);
  }
}
...
module M1046(){
  color("silver")difference(){
    union(){
      ...
      up(2.75)rot(-5)spur_gear(circ_pitch=1.9, teeth=17, thickness=5.5, shaft_diam=3.7);color("Silver");

    }
    translate([0,0,-1])cylinder(20,d=3.8);
  }
  translate([0,-1.5,9])rotate([90,0,0])B2_WM4();
}
...
module M1047(){
  ...
  color("black")difference(){
    union(){
      cylinder(h=2.5,d=49);
      up(2.5/2)rot(1)spur_gear(circ_pitch=1.89, teeth=83, thickness=2.5, shaft_diam=30);
    }
    T0_W6(2.5);
  }
  translate([0,0,0])B2_hub(10);
}
...
module M1049(){
  color("Silver"){
    difference(){
      union(){
        cylinder(h=20.2,d=8);
        translate([0,-1,5])rotate([90,0,0])B2_WM4();
        translate([0,-1,16])rotate([90,0,0])B2_WM4();
        translate([-1,0,10])rotate([0,-90,0])B2_WM4();
      }
      union(){
...
module M1050(){
  color("Silver"){
    translate([0,-1.3,3.8])rotate([90,0,0])B2_SM3();
    difference(){
...
//  1051 - nut M3.5
//  $fn=30;M1051();translate([20,0,0])M1051("thin");
module M1051(thn="normal"){
  color("Silver")B2_M35N(thn);
}

//  1052 - screw M3.5x6
//  $fn=30;M1052(1);
module M1052(dx=30,thn="normal"){
    color("silver")B2_M35(6,dx,thn);
}

//  1053 - screw M3.5x8
//  $fn=30;M1053(6,"thin");
module M1053(dx=30,thn="normal"){
    color("silver")B2_M35(8,dx,thn);
}

//  1054 - screw M3.5x16
//  $fn=30;M1054(6);
module M1054(dx=30,thn="normal"){
    color("silver")B2_M35(16,dx,thn);
}

//  1055 - screw M3.5x20
//  $fn=30;M1055(6);
module M1055(dx=30,thn="normal"){
    color("silver")B2_M35(20,dx,thn);
}
...
module M1056(){
  ...
  difference(){
    union(){
    color("DarkSlateGray")up(7.4)rot(3.6)bevel_gear(
    circ_pitch=1.55, teeth=25, mate_teeth=25,
    shaft_diam=3.7, spiral=0,cutter_radius=0,
    face_width=3.5);
    }
    cylinder(30,d=3.8);
  }
  translate([0,0,9])rotate([180,0,0])B2_hub(9);
}
...
up(11.7)rot(6.8)crown_gear(mod=.58, teeth=50, backing=3.2, face_width=2, pressure_angle=15);
    }
    //T0_W6(2.5);
    cylinder(10,d=3.8);
    translate([0,0,9])cylinder(5,d=30);
  }
  translate([0,0,10])rotate([180,0,0])B2_hub();
}

//  1068 crank
//  $fn=30;M1068();
module M1068(){
  move([0,-75,0])T0_shaft(75);
  translate([0,-89-12,16])T0_shaft(12);
  bez=[[0,-75,0],[0,-82,0],[0,-82,16],[0,-89,16]];
  bz_path=bezier_curve(bez);
  color("silver")stroke(bz_path,3.5);
}

//  1089 tyre for 1040
//  $fn=30;M1089("green",true);translate([45,0,0])M1089();
...
module M1092(h=25){
  hw=(h<20)?20:h;
    d_spring=.3;
  s1_path=helix(turns=60, h=hw, r=1);
    s2_path=helix(turns=1, h=0, r=1);
  translate([0,0,1])color("Gray"){
    stroke(s1_path,d_spring);
    down(1)xrot(90)stroke(s2_path,d_spring);
    up(hw+1)xrot(90)stroke(s2_path,d_spring);
  }
}
...
module M1094(h=33){
  hw=(h<5)?5:h;
     d_spring=.5;
    s1_path=helix(turns=7, h=hw, r=6.5);
    s2_path=helix(turns=1, h=0, r=6.5);
  color("Gray"){
    stroke(s1_path,d_spring);
    stroke(s2_path,d_spring);
    up(hw)stroke(s2_path,d_spring);
  }
}
...
module M1140(angle1=0,angle2=0){
  ...
  module T0_1140A(){
    translate([0,0,-20])union(){
      translate([0,0,9])rotate([0,180,0])B2_hub(9);
      color("Silver"){
        difference(){
          union(){
...
module M1251(dx=3,thn="normal"){
    color(T0_brass)B2_M35(4,dx,thn,drive="flat");
}

//  1252 - screw M3.5x6
//  $fn=30;M1252(10);
module M1252(dx=3,thn="normal"){
    color(T0_brass)B2_M35(6,dx,thn,drive="flat");
}

//  1253 - screw M3.5x8
//  $fn=30;M1253(1);
module M1253(dx=3,thn="normal"){
    color(T0_brass)B2_M35(8,dx,thn,drive="flat");
}

//  1254 - screw M3.5x12
//  $fn=30;M1254(1);
module M1254(dx=3,thn="normal"){
    color(T0_brass)B2_M35(12,dx,thn,drive="flat");
}

//  1255 - screw M3.5x16
//  $fn=30;M1255(1);
module M1255(dx=3,thn="normal"){
    color(T0_brass)B2_M35(16,dx,thn,drive="flat");
}

//  1256 - screw M3.5x22
//  $fn=30;M1256(1);
module M1256(dx=3,thn="normal"){
    color(T0_brass)B2_M35(22,dx,thn,drive="flat");
}
...
module M2046(){
  color("silver")difference(){
    union(){
      cylinder(h=1.8,d=18);
       color("Silver")up(2.5/2)rot(5)spur_gear(circ_pitch=1.89, teeth=34, thickness=2.5, shaft_diam=3.7);

    }
    translate([0,0,-1])cylinder(20,d=3.8);
  }
  translate([0,0,-0.5])B2_hub(10.2);
}
...
module M2047(){
  difference(){
    union(){
      ...
      color("black")up(2.5/2)rot(2.3)spur_gear(circ_pitch=1.884, teeth=117, thickness=2.5, shaft_diam=60);

    }
    color("silver")translate([0,0,-1])cylinder(20,d=3.8);
  }
  translate([0,0,-.1])B2_hub(10.1);
}
  ...
  color("Silver"){
    difference(){
      union(){
        ...
        translate([0,-1,4])rotate([90,0,0])B2_WM4();
        translate([-1,0,10])rotate([0,-90,0])B2_WM4();
      }
      union(){
        translate([0,0,-0.05])cylinder(21,d=3.8);
...
module M2053(dx=30,thn=T0_nnrm){
    color("silver")B2_M35(10,dx,thn);
}

//  2054 - screw M3.5x22
//  $fn=30;M2054();
module M2054(dx=30,thn=T0_nnrm){
    color("silver")B2_M35(22,dx,thn);
}
...
module M2089(){
  ...
  translate([0,0,23])B2_hub(wm4=true); //8mm nad povrch
}
...
// ex: $fn=30; B2_screw_head("flat");
module B2_screw_head(drive="torx"){

  difference(){
    cyl(l=2.33,d1=5.9,d2=5.9,anchor=BOTTOM,rounding=.25);
    if(drive=="flat")up(2)cube([1,7,2],center=true);
    else up(2.85-1.8)linear_extrude(6)torx_mask2d(20,CENTER,0);
  }
}
...
////    thickness - nut thick, tloušťka matice = "normal"|"thin"
//  );
//  if "dist > len" no render nut
//  pokud "dist > len" nerenreruje matici
// ex:
//  $fn=30;translate([0,0,10.5])B2_M35(10,5);translate([10,0,0])B2_M35(10,5,thickness="thin");
module B2_M35(shaftlen=4,nutdist=30,thickness="normal",drive="torx"){
    translate([0,0,-shaftlen]){
      screw("M3.5",length=shaftlen+1,head="none",anchor=BOTTOM);
      thn=(thickness=="normal")?T0_nnrm:T0_nthc;
      translate([0,0,shaftlen])B2_screw_head(drive);
      if(shaftlen>nutdist)translate([0,0,shaftlen-(nutdist+thn)])B2_M35N(thickness);
    }
}
...
//  );
// ex:
//  $fn=30;B2_M35N();translate([20,0,0])B2_M35N("thin");
module B2_M35N(thickness="normal"){
  nut("M3.5",thickness=thickness);
}
